...
<div class="space-y-6">
    ...
    <p class="text-xs text-text-muted">The user only ever receives the Net amount — the Fee is credited to the platform's Primary Wallet on completion. <a href="{{ route('admin.settings.withdrawal-fee') }}" class="text-brand hover:underline">Adjust the withdrawal fee</a>.</p>

    <div class="glass-card overflow-x-auto">
        <table class="data-table">
            <thead><tr><th>User</th><th>Asset</th><th>Amount</th><th>Fee</th><th>Net (send this)</th><th>Send Via</th><th>Destination</th><th>Status</th><th>Note</th><th></th></tr></thead>
            <tbody>
                @forelse ($withdrawals as $w)
                    <tr>
                        ...
                        <td class="font-numeric font-semibold">{{ number_format($w->net_amount ?: ($w->amount - $w->fee), 8) }}</td>
                ...
                @empty
                    <tr><td colspan="10" class="text-center text-text-muted">No withdrawals yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
