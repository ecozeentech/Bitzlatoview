...
<div class="space-y-6">
    ...
    <div class="glass-card p-5">
        ...
        <div class="overflow-x-auto">
            <table class="data-table">
                <thead><tr><th>Date</th><th>Asset</th><th>Amount</th><th>Status</th><th>Note</th></tr></thead>
                <tbody>
                    @forelse ($deposits as $d)
                        <tr>
                            ...
                            <td><span class="pill-{{ $d->status === 'credited' ? 'success' : ($d->status === 'rejected' || $d->status === 'failed' ? 'danger' : 'warning') }}">{{ $d->status }}</span></td>
                            <td class="text-text-muted">{{ $d->user_note }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="text-center text-text-muted">No deposits yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="glass-card p-5">
        ...
        <div class="overflow-x-auto">
            <table class="data-table">
                <thead><tr><th>Date</th><th>Asset</th><th>Amount</th><th>Fee</th><th>Status</th><th>Note</th></tr></thead>
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center text-text-muted">No withdrawals yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
