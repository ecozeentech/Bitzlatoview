...
<div class="space-y-6">
    ...
    <div class="risk-banner"><strong class="text-danger">Not a real, spendable payment card yet.</strong> No licensed card-issuing provider is connected. Real cards must be issued by a regulated bank partner under a Visa/Mastercard license (e.g. via Stripe Issuing, Marqeta or Lithic) — this section will only produce usable cards once that integration is complete. All requests are reviewed by an admin before activation.</div>

    @if (! $issuingEnabled)
        <div class="glass-card p-5 text-sm text-text-muted">Card issuing is temporarily unavailable. Please check back later.</div>
    ...
    @else
        <div class="glass-card p-6">
            <h2 class="mb-3 font-semibold">Request a Virtual Card</h2>
            <p class="mb-3 text-xs text-text-muted">
                Max limit ${{ number_format($settings->max_spending_limit) }}.
                @if ($settings->issuance_fee > 0) Issuance fee ${{ number_format($settings->issuance_fee, 2) }}. @endif
                @if ($settings->funding_fee_pct > 0) Funding fee {{ $settings->funding_fee_pct }}%. @endif
                @if ($settings->monthly_fee > 0) Monthly fee ${{ number_format($settings->monthly_fee, 2) }}. @endif
            </p>
            <form method="POST" action="{{ route('app.virtual-cards.store') }}" class="grid gap-3 sm:grid-cols-3">
                ...
                <input type="number" step="0.01" max="{{ $settings->max_spending_limit }}" name="spending_limit" class="input-field" placeholder="Spending limit" required>
                <select name="currency" class="input-field">
                    @foreach ($settings->allowed_currencies as $cur)
                        <option value="{{ $cur }}">{{ $cur }}</option>
                    @endforeach
                </select>
                <button class="btn-brand sm:col-span-3">Request Card</button>
            </form>
        </div>
    @endif
        ...
        @foreach ($cards as $card)
            <div class="glass-card p-5" x-data="{ revealed: false, number: '', cvv: '' }">
                ...
                <div class="mt-3 flex items-center justify-between text-sm">
                    <span class="pill-{{ $card->status === 'active' ? 'success' : ($card->status === 'frozen' ? 'warning' : ($card->status === 'pending' ? 'info' : 'muted')) }}">{{ ucfirst($card->status) }}</span>
                    <span class="text-text-muted">Limit: ${{ number_format($card->spending_limit, 2) }} {{ $card->currency }}</span>
                </div>

                @if ($card->status === 'pending')
                    <p class="mt-2 text-xs text-text-muted">Awaiting admin approval.</p>
                @elseif ($card->status === 'rejected')
                    <p class="mt-2 text-xs text-danger">Rejected: {{ $card->rejection_reason }}</p>
                @endif

                @if (in_array($card->status, ['active', 'frozen']))
                    <div class="mt-3 flex flex-wrap gap-2">
                        ...
                        <form method="POST" action="{{ route('app.virtual-cards.limit', $card) }}" class="flex gap-1">
                            @csrf
                            <input type="number" step="0.01" max="{{ $settings->max_spending_limit }}" name="spending_limit" class="input-field w-24 text-xs" placeholder="New limit">
                            <button class="btn-outline text-xs">Set Limit</button>
                        </form>
                    </div>
                @endif
                <form method="POST" action="{{ route('app.virtual-cards.cancel', $card) }}" class="mt-2">@csrf @method('DELETE')<button class="text-xs text-danger hover:underline">Cancel</button></form>
