...
<head>
    ...
    <title>@yield('title', 'Admin') — Bitzlatoview</title>
...
<body class="min-h-screen bg-background text-slate-50">
    <div class="flex min-h-screen">
        @include('partials.admin-sidebar')
        <div class="flex min-w-0 flex-1 flex-col">
            <header class="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-surface/90 px-6 py-4 backdrop-blur">
                <div>
                    <p class="text-xs uppercase tracking-[0.2em] text-brand">Admin Control</p>
                    <h1 class="text-lg font-semibold">@yield('title', 'Dashboard')</h1>
                </div>
                <a href="{{ route('app.dashboard') }}" class="btn-outline text-xs">User App</a>
            </header>
